...
async def _build_settings_out(db: AsyncSession) -> SettingsOut:
    ...
    return SettingsOut(
        ...
        wa_auto_enabled=s.get("wa_auto_enabled") or "0",
        wa_notify_statuses=s.get("wa_notify_statuses") or "",
...
async def whatsapp_status(
    ...
    """Connection state + QR (data URI) from the WhatsApp bridge."""
    return await whatsapp_service.bridge_status()
...
async def whatsapp_test(
    ...
    """Send a test WhatsApp message to a number to verify the link works."""
    s = await get_all_settings(db)
    number = whatsapp_service.normalize_msisdn(to, s.get("whatsapp_country_code") or "27")
    if not number:
        raise HTTPException(status_code=400, detail="Enter a valid phone number")
    company = s.get("company_name") or "WorkshopIQ"
    text = f"Test message from {company} — WorkshopIQ WhatsApp notifications are working."
    ok, err = await whatsapp_service.send_message(number, text)
    if not ok:
